...
abstract public class PictureImpl implements Picture {
	...
	private static Pixel findAverage(Picture pic, int x, int y, Pixel average, double factor, int x_total, int y_total) {
		if (x_total < factor && x < pic.getWidth()) {
			Pixel blend = average.blend(pic.getPixel(x + 3, y), .5);
			return findAverage(pic, x + 3, y, blend, factor, x_total + 2, y_total);
		} else if (y_total < factor && y < pic.getHeight()) {
			Pixel blend = average.blend(pic.getPixel(x, y + 3), .5);
			return findAverage(pic, x, y + 3, blend, factor, x_total, y_total + 2);
		} else {
			return average;
		}
	}
	...
	public Picture blur(int ax, int ay, int bx, int by, Pixel p, double factor, int blur_factor) {
		...
		for (int x = min_x; x <= max_x; x++) {
			for (int y = min_y; y <= max_y; y++) {
				Pixel _p = this.getPixel(x,y);

				_p = findAverage(this, x, y, _p, blur_factor, 0, 0);
				_p = _p.blend(p, factor);

				result = result.paint(x,y,_p);
			}
		}
		return result;
	}
